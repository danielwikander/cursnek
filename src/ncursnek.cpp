...
struct hhscore {
	int hscore;
	string hsname;
	bool operator < (const hhscore& rhs) const {
...
ncursnek::ncursnek() 
{
  // Get window size for dynamic gamemap 
  //getWindowSize(beg_y, beg_x, max_y, max_x); // Sets the window size variables
  // Create gamemap and fill with emptiness
  int row, col;						
  ...
  WINDOW *win = newwin(sizey - 2, sizex, 1, 2);
  nodelay(win, true);
  notimeout(win, true);
  box(win, 0, 0);
  mvprintw(sizey, 3, "Navigate with wasd/hjkl. Press q to exit.");

  // Start the gameloop
  gameLoop(win, gamemap, snekCoordinates);
}

void ncursnek::getWindowSize(int &beg_y, int &beg_x, int &max_y,
                                 int &max_x) {
  getbegyx(stdscr, beg_y, beg_x);
  getmaxyx(stdscr, max_y, max_x);
}

void ncursnek::setUpSnek(int startx, int starty, mapcontent gamemap[][gamegridysize],
                             deque<Coordinate> &snekCoordinates) {
...
void ncursnek::gameLoop(WINDOW *win,
                            mapcontent gamemap[][gamegridysize],
                            deque<Coordinate> &snekCoordinates) {
...
bool ncursnek::collisionCheck(mapcontent gamemap[][gamegridysize],
  ...
  if (nextStep.x < 0 || nextStep.y < 0 || nextStep.x >= gamegridxsize ||
    ...
    fatalCollision = false;
    Coordinate tail = Coordinate(snekCoordinates.back().x, snekCoordinates.back().y);
...
void ncursnek::showHighScores(vector<string> topscores) {
  ...
  mvwprintw(scorewin, 0, 20, "Press q to exit");
  curs_set(0);
  wrefresh(scorewin);
